...
- 클래스 프리픽스(adr-a4-)와 컬러 토큰, 결재란/브랜드 마크/번호 섹션 구조를 그대로 재사용하여
  Wivo 원무일일보고서(daily-report-app)와 시각적으로 통일감 있게 제작.
- 인쇄(Ctrl+P) 시 A4 1~2페이지에 맞도록 @page 규칙 적용
- 로고는 CSS로 직접 그린 브랜드 마크(원무일일보고서와 동일)를 사용하여 이미지 없이도 가볍게 완결
"""
import base64
...
def build_html_report(ctx: dict) -> str:
    ...
    night_summary = " + ".join(it["구분"] for it in night_items) if night_items else "선택 안 함"
...
<main class="adr-a4-report">
  <header class="adr-a4-head">
    ...
    <div class="adr-a4-header-grid">
      ...
      <section class="adr-a4-approval">
        ...
        <div class="adr-a4-approval-grid"><div>담당</div><div>실장</div><div>원장</div></div>
      </section>
    </div>
    <div class="adr-a4-meta">
      ...
      <div><small>ANALYSIS</small><b>MEDIEM AI</b></div>
    </div>
  </header>
  <div class="adr-a4-content">
    ...
    <section class="adr-a4-section">
      ...
      <div class="adr-a4-ai-box"><strong>MEDIEM AI CONSULTING NOTE</strong>{ai_html}</div>
    </section>
