...
- **병실료** 시트: `명칭 | 등급 | 수가코드_1~15일 | 금액_1~15일 | 수가코드_16~30일 | 금액_16~30일 | 수가코드_31일이상 | 금액_31일이상`
  - `명칭`은 "병원 2등급간호관리료적용 2인실입원료"처럼 실제 청구 항목명을 그대로 입력합니다.
  - `등급`은 숫자 등급(1~6 등) 또는 'A' 등 병원에서 쓰는 표기를 문자열로 입력합니다.
  - 명칭에 "N인실"이 포함되어 있으면 등급별 수익 비교 시 동일 병실 크기로 자동 매칭됩니다.
- **야간간호료** / **야간전담간호료** 시트: `명칭 | 수가코드 | 1일당_금액` (등급 없이, 병원이 실제 운영 중인 항목을 나열)
  - 예: 야간전담간호사 운영비율 구간별(10% 미만 / 10~15% / 15~20% / 20~25% / 25% 이상)로 여러 행 작성
  - 보고서 작성 시 이 중 실제 적용 중인 항목 1개를 선택해서 계산합니다.
